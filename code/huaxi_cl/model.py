...
class MobileNetV2(nn.Module):
    def __init__(self, num_classes=2, width_mult=1.0, inverted_residual_setting=None, round_nearest=1):
        """
    ...
    def forward(self, x, Clinical_features):
        ...
        x = self.drop(x)
        x = x.view(x.size(0), -1)
        clf = self.clf_fc(Clinical_features)
        #print(x.size())
        x = torch.cat([x, clf], dim=1)
        #print(x.size())
...
def mobilenet_v2(pretrained=False, **kwargs):
    ...
    if pretrained:
        pretrained_dict = torch.load('/media/stianci/360b04be-f526-4e91-aad3-85535b17af3e/twx/pywork/pytorch/3D_class/mobilenet/huaxi/models4/130mobilev2.pth').state_dict()
